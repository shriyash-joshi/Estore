...
class StringUtils {
    ...
    public static function excerpt($text, $phrase, $radius = 100, $ending = '...') {
        if(empty($text) || empty($phrase)) {
            return self::truncate($text, $radius * 2, ['ending' => $ending]);
        }
    ...
    public static function absInt($value) {
        return is_array($value) ? array_map(function($v) { return (int)trim($v); }, $value) : (int)$value;
    }

    public static function numbnerToHuman($number, $decimals = 1) {

        switch(strlen(round($number, 0))) {
            case 1:
            case 2:
            case 3:
                return [$number, ''];
