...
function pmxe_wp_ajax_wpae_preview(){
	...
	else
	{
		...
		elseif( in_array('comments', $exportOptions['cpt']))
		{
			...
			{
				$exportQuery = new WP_Comment_Query( array('type__not_in' => ['review'], 'orderby' => 'comment_ID', 'order' => 'ASC', 'number' => 10 ));
			}
			else
			{
				$exportQuery = get_comments( array('type__not_in' => ['review'], 'orderby' => 'comment_ID', 'order' => 'ASC', 'number' => 10 ));
			}
        ...
        elseif( in_array('shop_review', $exportOptions['cpt']))
        {
            ...
            if ( version_compare($wp_version, '4.2.0', '>=') )
            {
                $exportQuery = new WP_Comment_Query( array('type' => 'review', 'orderby' => 'comment_ID', 'order' => 'ASC', 'number' => 10 ));
            }
            else
            {
                $exportQuery = get_comments( array('type' => 'review', 'orderby' => 'comment_ID', 'order' => 'ASC', 'number' => 10 ));
            }
            remove_action('comments_clauses', 'wp_all_export_comments_clauses');
