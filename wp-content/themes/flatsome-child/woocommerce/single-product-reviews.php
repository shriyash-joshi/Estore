...
$review_ratings_enabled = fl_woocommerce_version_check( '4.3.0' ) ? wc_review_ratings_enabled() : get_option( 'woocommerce_enable_review_rating' ) === 'yes';
?>
<h2 class="fontsize30">Reviews</h2>
	...
	<?php if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) : ?>
		...
		<div id="review_form_wrapper" class="large-<?php if ( get_comment_pages_count() == 0 || $tab_style == 'sections' || $tab_style == 'tabs_vertical' ) { echo '12'; } else { echo '5'; } ?> col">
			<div id="review_form" class="col-inner">
				...
				$comment_form['comment_field'] .= '<p class="comment-form-comment text-danger" style="color:red;" id="review-comment"><label for="comment">' . esc_html__( 'Your review', 'woocommerce' ) . '&nbsp;<span class="required">*</span></label><textarea id="comment" name="comment" cols="45" rows="8" required></textarea></p>';
...
jQuery('#submit').on('click',function(e){
   ...
   var commentContent = jQuery("#comment").val();
    if(commentContent ==""){
        jQuery('#review-comment').text('Please Enter Review');
	}
});
</script>